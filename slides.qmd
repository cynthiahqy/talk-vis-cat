...
### Proposed Task Abstraction & Visualisation
  
- Cross-Taxonomy Transformation and Crossmaps
...
**Documenting and communicating the chosen data transformations**.
:::
:::
...
What are the important features of the data transformation task?
...
Given a defined mapping, we still need to **implement** the transformation.
...
@zhouMatrixMultiplicationSQL2020 shows that matrix multiplication can be implemented using database join, mutate and summarise operations. Thus implementation can be done as a two-input function:
...
[Under the hood `apply_xmap`:]{.fragment}

1. **Validates** transformation logic
2. Checks the crossmap **fully covers** the source data taxonomy
3. **Transforms** the data according the crossmap logic
:::
...
[Key transformation attributes:]{.fragment}
...
## Effective Communication of Preprocessing
...
- Crossmaps allow us to **visualise** the extent of data imputation and **quantify** the degree of imputation
:::
:::
:::
...
::: {.callout-tip}
## Final remarks
- Visualisation can be used to communicate complex data wrangling decisions
- Designing visualisations can lead to surprising statistical insights
:::

::: {.callout-note}
## Useful links
  - Working paper on visualising crossmaps: <https://arxiv.org/abs/2308.06535>
  - Interest sign-up
  - Slides:
:::
